...
pub fn make_types() {
    ...
    std::fs::write("../src/autogenerated/particle_types.rs", fmt).unwrap();
}

pub(crate) fn make_types_to_json() -> Vec<(String, u32)> {
    let mut registries = get_reports_json();
    let items = registries.map.get_mut("minecraft:particle_type").unwrap();
...
pub fn make_types_tokens(item_entries: Vec<(String, u32)>) -> TokenStream {
    ...
    for entry in item_entries {
        ...
        stream.extend(quote::quote! {
            registry.insert(Identifier::vanilla_const(#name), ParticleType { id: Identifier::vanilla_const(#name) });
        });
    }
    quote::quote! {
        use crate::packet::*;
