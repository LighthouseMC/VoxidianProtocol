...
impl NbtCompound {
    ...
    pub fn get_mut<S: AsRef<str>>(&mut self, key: S) -> Option<&mut NbtElement> {
...
impl fmt::Debug for NbtCompound {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{:?}", self.0)
    }
}
impl NbtCompound {
    pub(super) fn encode_packet(&self, buf: &mut PacketBuf) {
...
impl NbtCompound {
    pub(super) fn decode_packet(buf: &mut PacketBuf) -> Result<Self, DecodeError> {
        let mut compound = Self::new();
        while (buf.remaining() > 0) {
            let tag = buf.read_u8()?;
            if (tag == NbtElement::TAG_END) {
                break;
            }
