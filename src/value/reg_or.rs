...
impl<T, U : PacketEncode> PacketEncode for RegOr<T, U> { fn encode(&self, buf : &mut PacketBuf) -> Result<(), EncodeError> {
    match (self) {
        #[allow(deprecated)]
...
impl<T, U : PacketDecode> PacketDecode for RegOr<T, U> { fn decode(buf : &mut PacketBuf) -> Result<Self, DecodeError> {
