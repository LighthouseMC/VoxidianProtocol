...
pub fn packet(attr : TokenStream, item : TokenStream) -> TokenStream {
    ...
    match (&input) {
        ...
        Item::Struct(ItemStruct { ident, fields, .. }) => {
            if (! attr.is_empty()) {
                Span::call_site().error("Attribute not allowed on struct packets").emit();
                item
            } else {
        ...
        Item::Enum(ItemEnum { ident, variants, .. }) => {
            ...
            let discriminant_ident = parse_str::<Ident>(&format!("__{}Discriminant", ident)).unwrap();

            let mut encode = quote!{ };
            let mut decode = quote!{ let discriminant = Into::<i32>::into(buf.read_decode::<#repr>()?); };
            for Variant { ident, fields, .. } in variants {
                ...
                decode.extend(quote!{ if (#discriminant_ident::#ident as i32 == discriminant) {
