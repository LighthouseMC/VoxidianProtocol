    ...
    match (&input) {
        ...
        Item::Enum(ItemEnum { ident, variants, .. }) => {
            ...
            (quote!{
                ...
                impl PacketDecode for #ident { fn decode<'l>(buf : &mut PacketReader<'l>) -> Result<Self, DecodeError> { #decode Err(DecodeError::InvalidData(Cow::Borrowed("Packet enum discriminant not in range"))) } }
            }).into()
